...
function onWriteComplete(status) {
  ...
  const req = stream._httpMessage;
  const error = status < 0 ?
    new ErrnoException(status, 'write', this.error) : undefined;

  if (req?._debugWritableFinished) {
    process._rawDebug('[writable-finished-debug]', {
      phase: 'native-write-complete',
      status,
      code: error?.code,
      wrapError: this.error,
      callbackPresent: typeof this.callback === 'function',
      ...
      requestOutputSize: req.outputSize,
      requestWritableLength: req.writableLength,
      socketWritableLength: stream.writableLength,
      socketDestroyed: stream.destroyed,
      socketHadError: stream._hadError,
    });
  }
