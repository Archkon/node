...
const detailedDebug = createDetailedLogger('inspect_client', debuglog);
...
class Client extends EventEmitter {
  constructor() {
    ...
    this.trace = createTrace('inspect_client');
  ...
  _handleChunk(chunk) {
    ...
    while (this._unprocessed.length > 2) {
      ...
      if (id) {
        const pendingMethod = this._pendingMethods[id];
        this.trace.record('response', {
          id,
          method: pendingMethod,
          error: error?.code ?? error?.message,
          pending: ObjectKeys(this._pending).length,
          resultKeys: result === undefined ? [] : ObjectKeys(result),
        });
        detailedDebug('< response id=%s pending=%s error=%j resultKeys=%j',
                      id, ObjectKeys(this._pending).length,
                      error, result === undefined ? [] : ObjectKeys(result));
        ...
        detailedDebug('< event %s params=%j pending=%s',
                      method, params, ObjectKeys(this._pending).length);
  ...
  reset() {
    ...
    this.trace.record('reset', {
      pending: pending === undefined ? 0 : ObjectKeys(pending).length,
      hasSocket: this._socket !== undefined && this._socket !== null,
      hasHttp: this._http !== undefined && this._http !== null,
    });
    if (pending) {
  ...
  callMethod(method, params) {
    return new Promise((resolve, reject) => {
      ...
      this._pendingMethods[data.id] = method;
      const json = JSONStringify(data);
      debuglog('> %s', json);
      this.trace.record('request', {
        id: data.id,
        method,
        pending: ObjectKeys(this._pending).length,
        bytes: Buffer.byteLength(json),
      });
      detailedDebug('> request id=%s method=%s params=%j pending=%s bytes=%s',
                    data.id, method, params, ObjectKeys(this._pending).length,
  ...
  async connect(port, host) {
    this._port = port;
    this._host = host;
    this.trace.record('connect', { port, host });
  ...
  async _discoverWebsocketPath() {
    ...
    this.trace.record('discoveredWebSocket', { pathname, search });
  ...
  _connectWebsocket(urlPath) {
    ...
    httpReq.on('error', (e) => {
      this.trace.record('httpError', { message: e.message, code: e.code });
      this.emit('error', e);
    });
    ...
    const handshakeListener = (res, socket) => {
      ...
      this.trace.record('websocketUpgrade', { urlPath });

      this._socket = socket;
      socket.on('data', this.handleChunk);
      socket.on('close', () => {
        this.trace.record('websocketClose', {
          pending: ObjectKeys(this._pending).length,
        });
        this.emit('close');
      });
