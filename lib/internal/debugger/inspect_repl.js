...
const { createTrace } = require('internal/debugger/inspect_trace');
const { waitForDebugger } = require('internal/debugger/inspect_helpers');
const debuglog = require('internal/util/debuglog').debuglog('inspect');
const detailedDebug = createDetailedLogger('inspect_repl', debuglog);
...
function createRepl(inspector) {
  ...
  const replTrace = createTrace('inspect_repl');
  ...
  function resetOnStart() {
    replTrace.record('resetOnStart', {
      hadExitDebugRepl: !!exitDebugRepl,
      hadInitialBreakRender: !!initialBreakRender,
  ...
  function setPauseOnExceptions(state) {
    detailedDebug('setPauseOnExceptions start state=%s previous=%s',
                  state, pauseOnExceptionState);
    replTrace.record('setPauseOnExceptionsBegin', {
      state,
      previous: pauseOnExceptionState,
    });
    return PromisePrototypeThen(
      Debugger.setPauseOnExceptions({ state }),
      () => {
        pauseOnExceptionState = state;
        detailedDebug('setPauseOnExceptions done state=%s', state);
        replTrace.record('setPauseOnExceptionsDone', { state });
      });
  }
  ...
  function createInitialBreakRenderPromise(pauseRender) {
    ...
    detailedDebug('createInitialBreakRenderPromise');
    replTrace.record('initialBreakRenderPromiseCreated');
  ...
  Debugger.on('paused', ({ callFrames, reason, hitBreakpoints }) => {
    ...
    detailedDebug(
      'paused event reason=%s frames=%s hitBreakpoints=%j waitInitial=%s initialPending=%s top=%j',
      reason, callFrames?.length ?? 0, hitBreakpoints,
      waitForInitialBreakRender, !!initialBreakRender,
      topFrame?.location);
    replTrace.record('paused', {
      reason,
      hitBreakpoints,
      frames: callFrames?.length ?? 0,
      waitForInitialBreakRender,
      hasInitialBreakRender: !!initialBreakRender,
  ...
  function handleResumed() {
    detailedDebug('resumed event');
    replTrace.record('resumed');
    currentBacktrace = null;
    selectedFrame = null;
  }
  ...
  Debugger.on('scriptParsed', (script) => {
    ...
    detailedDebug('scriptParsed scriptId=%s url=%s context=%s isModule=%s length=%s',
                  scriptId, url, script.executionContextId,
                  script.isModule, script.length);
    if (url && !isNativeUrl(url)) {
      replTrace.record('scriptParsed', {
        scriptId,
        url,
  ...
  async function runAndInit() {
    detailedDebug('runAndInit begin');
    replTrace.record('runAndInitBegin');
    await inspector.run();
    detailedDebug('runAndInit inspector.run complete');
    replTrace.record('runAndInitRunComplete');
    await initAfterStart();
    detailedDebug('runAndInit initAfterStart complete');
    replTrace.record('runAndInitInitComplete');
  }
  ...
  async function initAfterStart() {
    ...
    detailedDebug('initAfterStart begin script=%s waitInitial=%s pauseState=%s',
                  inspector.options?.script, waitForInitialBreakRender,
                  pauseOnExceptionState);
    replTrace.record('initAfterStartBegin', {
      hasScript: !!inspector.options?.script,
      waitForInitialBreakRender,
      pauseOnExceptionState,
    });
    if (waitForDebuggerOnStart) {
      detailedDebug('initAfterStart awaiting NodeRuntime.waitingForDebugger');
      replTrace.record('waitForDebuggerBegin');
      await waitForDebugger(inspector.client);
      detailedDebug('initAfterStart target startup gate is waiting');
      replTrace.record('waitForDebuggerDone');
    }
    await Runtime.enable();
    detailedDebug('initAfterStart Runtime.enable complete');
    replTrace.record('runtimeEnableDone');
    await Profiler.enable();
    detailedDebug('initAfterStart Profiler.enable complete');
    replTrace.record('profilerEnableDone');
    await Profiler.setSamplingInterval({ interval: 100 });
    detailedDebug('initAfterStart Profiler.setSamplingInterval complete');
    replTrace.record('profilerSamplingDone');
    await Debugger.enable();
    detailedDebug('initAfterStart Debugger.enable complete');
    replTrace.record('debuggerEnableDone');
    await Debugger.setAsyncCallStackDepth({ maxDepth: 0 });
    detailedDebug('initAfterStart Debugger.setAsyncCallStackDepth complete');
    replTrace.record('asyncCallStackDepthDone');
    await Debugger.setBlackboxPatterns({ patterns: [] });
    detailedDebug('initAfterStart Debugger.setBlackboxPatterns complete');
    replTrace.record('blackboxPatternsDone');
    await Debugger.setPauseOnExceptions({ state: pauseOnExceptionState });
    detailedDebug('initAfterStart Debugger.setPauseOnExceptions complete state=%s',
                  pauseOnExceptionState);
    replTrace.record('pauseOnExceptionsDone', { state: pauseOnExceptionState });
    await restoreBreakpoints();
    detailedDebug('initAfterStart restoreBreakpoints complete');
    replTrace.record('restoreBreakpointsDone');
    if (waitForDebuggerOnStart) {
      await inspector.client.callMethod('NodeRuntime.disable');
      detailedDebug('initAfterStart NodeRuntime.disable complete');
      replTrace.record('nodeRuntimeDisableDone');
    }
    detailedDebug('initAfterStart Runtime.runIfWaitingForDebugger begin');
    replTrace.record('runIfWaitingForDebuggerBegin');
    await Runtime.runIfWaitingForDebugger();
    detailedDebug('initAfterStart Runtime.runIfWaitingForDebugger complete');
    replTrace.record('runIfWaitingForDebuggerDone');
    await PromiseResolve();
    waitForInitialBreakRender = false;
    const initialBreakRenderPromise = initialBreakRender;
    initialBreakRender = null;
    detailedDebug('initAfterStart awaiting initialBreakRender=%s',
                  !!initialBreakRenderPromise);
    replTrace.record('initialBreakRenderAwaitBegin', {
      hadInitialBreakRenderPromise: !!initialBreakRenderPromise,
    });
    await initialBreakRenderPromise;
    detailedDebug('initAfterStart complete');
    replTrace.record('initAfterStartDone');
  }

  return async function startRepl() {
    inspector.client.on('close', () => {
      replTrace.record('clientClose');
      resetOnStart();
    });

    // Init once for the initial connection
    replTrace.record('startReplBegin');
    await initAfterStart();
    replTrace.record('startReplInitComplete');
