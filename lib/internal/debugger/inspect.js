...
class NodeInspector {
  constructor(options, stdin, stdout) {
    ...
    this.stdin = stdin;
    this.stdout = stdout;
    this.trace = createTrace('inspect_cli');
    this.trace.record('created', {
      hasScript: !!options.script,
      isRemote: options.isRemote,
      host: options.host,
      port: options.port,
      scriptArgCount: options.scriptArgs.length,
    });
  ...
  async run() {
    this.trace.record('runBegin', {
      hasScript: !!this.options.script,
    ...
    this.child = child;
    this.trace.record('runTargetReady', {
      childPid: child?.pid,
      host,
      port,
    });
    ...
    for (let attempt = 0; attempt < 5; attempt++) {
      ...
      this.trace.record('connectAttempt', { attempt, host, port });
      this.stdout.write('.');
      try {
        await this.client.connect(port, host);
        debuglog('connection established');
        this.trace.record('connectSuccess', { attempt, host, port });
        this.stdout.write(' ok\n');
        return;
      } catch (error) {
        debuglog('connect failed', error);
        this.trace.record('connectError', {
          attempt,
          host,
          port,
          code: error.code,
    ...
    this.trace.record('connectFailed', { host, port });
    dumpAll('inspect-connect-failed');
  ...
  childPrint(text, which) {
    const hasDisconnectSentinel = StringPrototypeEndsWith(
      text,
      'Waiting for the debugger to disconnect...\n',
    );
    this.trace.record('childOutput', {
      which,
      bytes: text.length,
      hasDisconnectSentinel,
    });
...
function startInspect(argv = ArrayPrototypeSlice(process.argv, 2),
  ...
  function handleUnexpectedError(e) {
    dumpAll('inspect-uncaught-error');
