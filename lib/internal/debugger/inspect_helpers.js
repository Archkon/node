...
const helperTrace = createTrace('inspect_helpers');
...
async function waitForDebugger(
  ...
  const onWaiting = () => {
    helperTrace.record('waitForDebuggerReady');
    resolveWaiting();
  };
  const onClose = () => {
    helperTrace.record('waitForDebuggerClosed');
    rejectClosed(new ERR_DEBUGGER_ERROR(
      'Debugger session ended while waiting for target startup'));
  };

  helperTrace.record('waitForDebuggerBegin');
...
async function launchChildProcess(childArgs, inspectHost, inspectPort,
  ...
  helperTrace.record('launchBegin', {
    inspectHost,
    inspectPort,
    skipPortPreflight: !!options.skipPortPreflight,
    childArgCount: childArgs.length,
  });
  if (!options.skipPortPreflight) {
    await portIsFree(inspectHost, inspectPort);
    helperTrace.record('portFree', { inspectHost, inspectPort });
  }
  ...
  helperTrace.record('childSpawned', {
    pid: child.pid,
    inspectPort,
    childArgCount: childArgs.length,
  });
  ...
  return new Promise((resolve, reject) => {
    ...
    function onExit(code, signal) {
      helperTrace.record('childExitBeforeReady', {
        pid: child.pid,
        code,
        signal,
      });
    ...
    function onError(error) {
      helperTrace.record('childErrorBeforeReady', {
        pid: child.pid,
    ...
    function onStderr(text) {
      ...
      if (debug) {
        ...
        helperTrace.record('childReady', {
          pid: child.pid,
          actualHost: debug[1],
          actualPort: Number(debug[2]),
        });
