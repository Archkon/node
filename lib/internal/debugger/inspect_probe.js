...
const { createDetailedLogger } = require('internal/debugger/inspect_log');
const { createTrace, dumpAll } = require('internal/debugger/inspect_trace');
const detailedDebug = createDetailedLogger('inspect_probe', debug);
...
function shouldIncludeDiagnostics(terminal) {
  return process.env.NODE_INSPECT_TRACE_REPORT === '1' &&
...
class ProbeInspectorSession {
  constructor(options) {
    ...
    this.trace = createTrace('inspect_probe');
    this.child = null;
  ...
  finish(exitCode, terminal) {
    ...
    detailedDebug('finish detail: results=%s pending=%j childStatus=%s/%s cleanup=%s',
                  this.results.length, this.getPendingProbeIndices(),
                  this.child?.exitCode, this.child?.signalCode,
                  this.cleanupStarted);
    this.trace.record('finish', {
      exitCode,
      terminal: terminal?.event,
      pending: this.getPendingProbeIndices(),
  ...
  onChildOutput(text, which) {
    ...
    this.trace.record('childStderr', {
      bytes: text.length,
      hasListening: StringPrototypeIncludes(text, kProbeListeningPrefix),
      hasAttached: StringPrototypeIncludes(text, kProbeAttachedSentinel),
      hasDisconnect: StringPrototypeIncludes(combined, kProbeDisconnectSentinel),
    });
    ...
    if (this.connected &&
      ...
      this.trace.record('disconnectSentinel', {
  ...
  onClientClose() {
    ...
    detailedDebug('client close detail: connected=%s started=%s lastProbe=%s childStatus=%s/%s',
                  this.connected, this.started, this.lastProbeIndex,
                  this.child?.exitCode, this.child?.signalCode);
    this.trace.record('clientClose', {
      disconnectRequested: this.disconnectRequested,
      connected: this.connected,
      started: this.started,
      finished: this.finished,
      inFlight: this.inFlight,
      lastProbeIndex: this.lastProbeIndex,
      childExitCode: this.child?.exitCode,
      childSignalCode: this.child?.signalCode,
    });
    if (!this.connected) { return; }
  ...
  onPaused(params) {
    ...
    detailedDebug('paused detail: started=%s frames=%s topLocation=%j inFlight=%j',
                  this.started, params.callFrames?.length ?? 0,
                  topFrame?.location, this.inFlight);
    this.trace.record('paused', {
      finished: this.finished,
      started: this.started,
      reason: params.reason,
      hitBreakpoints: params.hitBreakpoints,
      frames: params.callFrames?.length ?? 0,
      topLocation: topFrame?.location,
      topUrl: topFrame?.url,
      inFlight: this.inFlight,
    });
  ...
  async handlePaused(params) {
    if (this.finished) {
      detailedDebug('handlePaused ignored: already finished');
      this.trace.record('handlePausedIgnored', { reason: 'finished' });
      return;
    }
    ...
    if (!this.started) {
      detailedDebug('handlePaused ignored: not started reason=%s', params.reason);
      this.trace.record('handlePausedIgnored', {
        reason: 'notStarted',
    ...
    if (hitBreakpoints === undefined || hitBreakpoints.length === 0) {
      detailedDebug('handlePaused resume: no hitBreakpoints reason=%s', params.reason);
      this.trace.record('handlePausedResume', {
        reason: 'noHitBreakpoints',
        pauseReason: params.reason,
      });
      await this.resume();
      return;
    ...
    if (callFrameId === undefined) {
      detailedDebug('handlePaused resume: missing callFrameId');
      this.trace.record('handlePausedResume', { reason: 'missingCallFrameId' });
      await this.resume();
      return;
    }
    ...
    for (const breakpointId of hitBreakpoints) {
      ...
      if (definition === undefined) {
        detailedDebug('handlePaused unknown breakpointId=%s location=%j',
                      breakpointId, location);
        this.trace.record('unknownBreakpoint', { breakpointId, location });
        continue;
      }
      detailedDebug('handlePaused breakpointId=%s probes=%j location=%j',
                    breakpointId, definition.probeIndices, location);
      this.trace.record('breakpointHit', {
        breakpointId,
    ...
    if (!this.finished && ArrayPrototypeSome(this.probes, probeReachedLimit)) {
      detailedDebug('handlePaused completing: max-hit reached');
      this.trace.record('maxHitReached');
      this.finishWithTrustedResult({ event: 'completed' });
      return;
    }

    detailedDebug('handlePaused resume after probes');
    this.trace.record('handlePausedResume', { reason: 'afterProbes' });
    await this.resume();
  }
  ...
  async evaluateProbe(callFrameId, probeIndex, location) {
    ...
    detailedDebug('evaluateProbe begin probe=%s hitBefore=%s expr=%j location=%j',
                  probeIndex, probe.hits, probe.expr, location);
    this.trace.record('evaluateProbeBegin', {
      probe: probeIndex,
      hitBefore: probe.hits,
      location,
    });
    ...
    detailedDebug('evaluateProbe done probe=%s hit=%s hasError=%s',
                  probeIndex, probe.hits, result.error !== undefined);
    this.trace.record('evaluateProbeDone', {
      probe: probeIndex,
      hit: probe.hits,
  ...
  async resume() {
    if (this.finished) { return; }
    detailedDebug('resume begin');
    this.trace.record('resumeBegin');
    await this.callCdp('Debugger.resume');
    detailedDebug('resume done');
    this.trace.record('resumeDone');
  }
  ...
  async callCdp(method, params, probe = null) {
    ...
    detailedDebug('CDP -> %s params=%j probe=%j started=%s finished=%s',
                  method, params, probe, this.started, this.finished);
    this.trace.record('cdpBegin', {
      method,
      probe,
      started: this.started,
      finished: this.finished,
    ...
    try {
      ...
      detailedDebug('CDP <- %s result=%j', method, result);
      this.trace.record('cdpSuccess', { method, probe });
      return result;
    } catch (err) {
      if (err !== kInspectorFailedSentinel) {  // Already handled.
        debug('CDP <- %s error: %s', method, err?.code);
      }
      this.trace.record('cdpError', {
        method,
        probe,
        sentinel: err === kInspectorFailedSentinel,
        code: err?.code,
        message: err?.message,
        disconnectRequested: this.disconnectRequested,
        finished: this.finished,
  ...
  recordInspectorFailure({ reason, advice, cdpError, internalError }) {
    ...
    this.trace.record('inspectorFailure', {
      reason,
      inFlight: this.inFlight,
      lastProbeIndex: this.lastProbeIndex,
      cdpErrorCode: cdpError?.code,
      cdpErrorMessage: cdpError?.message,
  ...
  attachListeners() {
    detailedDebug('attachListeners');
    this.trace.record('attachListeners');
  ...
  async bindBreakpoints() {
    ...
    detailedDebug('bindBreakpoints begin probes=%s', this.probes.length);
    this.trace.record('bindBreakpointsBegin', { probes: this.probes.length });
    ...
    for (const { target, condition, probeIndices } of uniqueTargets.values()) {
      ...
      detailedDebug('breakpoint bound: id=%s target=%j probes=%j condition=%j',
                    result.breakpointId, target, probeIndices, condition);
      this.trace.record('breakpointBound', {
    ...
    detailedDebug('bindBreakpoints complete count=%s', this.breakpointDefinitions.size);
    this.trace.record('bindBreakpointsDone', {
      count: this.breakpointDefinitions.size,
    });
  }
  ...
  buildReport({ exitCode, terminal }) {
    ...
    if (shouldIncludeDiagnostics(terminal)) {
      const traceFile = dumpAll(`probe-${terminal.event}`);
      report.diagnostics = {
        traceDirectory: process.env.NODE_INSPECT_DEBUG_LOG_DIR,
        targetPid: this.child?.pid,
        probeTrace: this.trace.snapshot(),
        clientTrace: this.client.getTrace(),
      };
      if (traceFile !== undefined) {
        report.diagnostics.traceFile = traceFile;
      }
    }
    return {
      code: exitCode,
  ...
  async cleanup() {
    ...
    this.trace.record('cleanupBegin', {
      childExitCode: this.child?.exitCode,
      childSignalCode: this.child?.signalCode,
    });
  ...
  async run() {
    try {
      ...
      detailedDebug('run begin childArgv=%j host=%s port=%s skipPortPreflight=%s',
                    childArgv, host, port, skipPortPreflight);
      this.trace.record('runBegin', { host, port, skipPortPreflight });
      const { 0: child, 1: actualPort, 2: actualHost } =
        await launchChildProcess(childArgv,
                                 ...
                                 { skipPortPreflight });
      this.child = child;
      detailedDebug('child launched pid=%s actualHost=%s actualPort=%s',
                    child.pid, actualHost, actualPort);
      this.trace.record('childLaunched', {
        pid: child.pid,
        actualHost,
        actualPort,
      });
      ...
      this.connected = true;
      detailedDebug('client connected actualHost=%s actualPort=%s', actualHost, actualPort);
      this.trace.record('clientConnected', { actualHost, actualPort });

      try {
        detailedDebug('waiting for target startup gate');
        this.trace.record('waitForDebuggerBegin');
        await waitForDebugger(
          this.client,
          (method) => this.callCdp(method),
        );
        detailedDebug('target startup gate is waiting');
        this.trace.record('waitForDebuggerDone');

        await this.callCdp('Runtime.enable');
        await this.callCdp('Debugger.enable');
        await this.bindBreakpoints();
        this.started = true;
        detailedDebug('startup complete, target about to run');
        this.trace.record('startupComplete');
        this.startTimeout();
        await this.callCdp('NodeRuntime.disable');
        this.trace.record('runIfWaitingForDebuggerBegin');
        await this.callCdp('Runtime.runIfWaitingForDebugger');
        detailedDebug('target released from Runtime.runIfWaitingForDebugger');
        this.trace.record('runIfWaitingForDebuggerDone');
      } catch (err) {
